...
## Core equations
- Probability ratio: $r_t(\theta) = \dfrac{\pi_\theta(a_t \mid s_t)}{\pi_{\text{old}}(a_t \mid s_t)}$
- Clipped surrogate (maximize; written as loss to minimize):  
  $\mathcal{L}_{\text{clip}} = - \mathbb{E}\Big[\min\big(r_t A_t,\ \text{clip}(r_t, 1-\epsilon, 1+\epsilon) A_t\big)\Big]$
- Advantage: typically $A_t = Q_t - V_t$ or GAE.
